...
var orders = [1, 2, 3, 4, 5, 6, 6, 13, 7, 12, 11, 10, 9, 8];
// 一次抽奖的时长
...
$(function () {
  ...
  $prizeItems.each(function (i, item) {
    $(item).html('<img src="' + prizes[i].img + '"><p class="img-mask" data-index="' + prizes[i].index + '"></p>');
  });
  ...
  function drawOne() {
    ...
    //请求接口，获取序号, 这里模拟
    var resultNum = parseInt(Math.random() * orders.length) || 1;
    console.log(resultNum);
    var current = parseInt(Math.random() * orders.length) || 1;
    function roll() {
    ...
    var interval = setInterval(roll, 50);
    setTimeout(function () {
